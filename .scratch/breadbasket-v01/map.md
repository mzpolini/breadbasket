...
- [The check-in conversation](issues/07-check-in-conversation.md) — The opening **names crops, never numbers** — anchoring risk lives in the figures, so every number on screen came out of his mouth this evening. The read-back leads with **kind**, then confidence, `[edit]` flips the kind first. No verbal hedge on ambiguity; the label carries it. The lapse nudge accepts one word. And the stocktake prompt becomes an **incentive rather than a reminder**: as estimate debt grows the public annotation degrades to `estimated · not weighed in 3 weeks`, which costs no interruption and puts the consequence where buyers see it. Artifact: [the transcripts](prototypes/conversation-draft-1.md).
- [The normalisation target shape](issues/06-normalisation-target-shape.md) — Three inference defaults, all erring toward under-claiming: **kind is absolute by default** ("I've got 50lb" is a true-up, `add` needs explicit *more/picked/another*), **`measured` is false by default** (flips only on *weighed/counted/scale* or a non-round figure), and **quantity is optional** — "I've got collards" is a valid presence-only movement. Requires a ledger change: `amount` becomes optional. Hard requirement dropped on the chat surface: the read-back must show the *kind* prominently, one tap to flip.
